...
create table if not exists public.map_events (
  ...
  discovery_mode text not null default 'public' check (discovery_mode in ('public', 'invisible')),
  affinity_interests text[] not null default '{}',
  affinity_goals text[] not null default '{}',
...
alter table public.map_events add column if not exists discovery_mode text not null default 'public';
alter table public.map_events add column if not exists affinity_interests text[] not null default '{}';
alter table public.map_events add column if not exists affinity_goals text[] not null default '{}';
